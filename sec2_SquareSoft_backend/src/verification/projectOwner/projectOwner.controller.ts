...
export class ProjectOwnerVerificationController {
  ...
  async projectOwnerReject(@Body("id") id: string) {
    ...
    await this.notificationService.createNotification({notificationType:"rejectedVerification",owner:id})
    await this.emailService.sendRejectedVerificationNotification(id);
    return result
  }
